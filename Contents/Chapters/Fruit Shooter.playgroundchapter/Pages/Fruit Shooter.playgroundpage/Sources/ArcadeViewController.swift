...
public class ArcadeViewController: UIViewController, ARSCNViewDelegate, PlaygroundLiveViewSafeAreaContainer {
    ...
    private let generationCycle : TimeInterval   = 2.0
    ...
    private var currentScore: Int = 0 {
        didSet {
            if !hasSucceeded && currentScore >= 100 {
                hasSucceeded = true
                PlaygroundPage.current.assessmentStatus = .pass(message: "You've got **100** points in **Fruit Shooter**, hope you have had fun in shooting fruits! 😆 Last but not the least, if you want a more challenging game, please adjust the gravity value on the 👈 left or below!")
                playSound(.success)
            }
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(100)) { [unowned self] in
            self.waitLabel.text = "Shoot the Fruits"
        }
